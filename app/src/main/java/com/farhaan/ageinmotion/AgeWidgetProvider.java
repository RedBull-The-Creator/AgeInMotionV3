...
public class AgeWidgetProvider extends AppWidgetProvider {
    ...
    static void update(Context c, AppWidgetManager m, int id) {
        ...
        int widthDp = m.getAppWidgetOptions(id).getInt(AppWidgetManager.OPTION_APPWIDGET_MIN_WIDTH, 110);
        boolean detailed = widthDp >= 170;
        v.setViewVisibility(R.id.detail_grid, detailed && !progressMode ? android.view.View.VISIBLE : android.view.View.GONE);
    ...
    private static void setAgeDetails(RemoteViews v, ZonedDateTime birth, ZonedDateTime now) {
        if (birth.isAfter(now)) {
            v.setTextViewText(R.id.d1, "0"); v.setTextViewText(R.id.d2, "0"); v.setTextViewText(R.id.d3, "0");
            v.setTextViewText(R.id.d4, "0"); v.setTextViewText(R.id.d5, "0"); v.setTextViewText(R.id.d6, "00.000"); v.setTextViewText(R.id.l6, "SECONDS.MS");
            return;
        }
        ...
        long hours = millis / 3_600_000L; millis %= 3_600_000L;
        long minutes = millis / 60_000L; millis %= 60_000L;
        long seconds = millis / 1_000L; long ms = millis % 1_000L;
        v.setTextViewText(R.id.d1, String.valueOf(p.getYears())); v.setTextViewText(R.id.d2, String.valueOf(p.getMonths())); v.setTextViewText(R.id.d3, String.valueOf(p.getDays()));
        v.setTextViewText(R.id.d4, String.valueOf(hours)); v.setTextViewText(R.id.d5, String.valueOf(minutes)); v.setTextViewText(R.id.d6, String.format(Locale.US, "%02d.%03d", seconds, ms));
        v.setTextViewText(R.id.l6, "SECONDS.MS");
    }
